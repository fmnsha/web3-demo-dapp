...
contract tweetsContract is Ownable {


    uint8 public TWEET_MAX_LENGTH = 10;
    ...
    function changeTweetLength(uint8 newLength) public onlyOwner {
        TWEET_MAX_LENGTH = newLength;
    }

    event TweetCreated ( uint256 indexed id, string content);

    function Add( string memory content) public {

        require(bytes(content).length <= TWEET_MAX_LENGTH,"too long tweet");
    ...
    function disLike(address author, uint256 id) external{
         require(tweets[author][id].id == id, "tweet not exist");
         require(tweets[author][id].likes > 0, "tweet not exist");
        tweets[author][id].likes -= 1;
    }
